  ...
  <ul class="lb-update-list">
    ...
    <li class="lb-update-item">
      ...
      <div class="lb-update-item__body">
        ...
        <pre class="lb-update-code"><code class="language-python">from lazybridge import Agent, LLMEngine, ReplanEngine, Store
from lazybridge.engines.replan import PlanRound

planner = Agent(
    engine=LLMEngine("claude-opus-4-8", system="You are a task planner."),
    output=PlanRound, name="planner",
)

# workers can be sub-agents or plain functions
...
guardian = Agent(
    engine=ReplanEngine(
        ...
        checkpoint_key="report",   # checkpoints every round
        resume=True,               # resume from the last round
        max_rounds=10,
    ),
    tools=[planner, research, writer],
    name="guardian",
)

guardian("Draft a staffing note on Apple vs Google.")
guardian("continue")   # resumes from the last round</code></pre>
